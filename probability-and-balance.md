...
Use McDie for dice; use a sheet for economies and set-collection curves (triangular default: 1, 3, 6, 10, 15).

## Common Balance Failure Modes

| Failure | Tell | Fix |
|---|---|---|
| Snowball / runaway leader | Early advantage compounds each turn | Split victory vs working currency; catch-up; declining returns |
| Kingmaking | Eliminated / hopeless player decides winner | Avoid elimination; hide scores; shared-victory options |
| Dominant strategy | One path wins across N playtests | Asymmetric costs; counters; RPS loops; orthogonal scoring |
| Turtling | Players avoid conflict, stalemate | Forced interaction; King of the Hill; escalating stakes; timer |
| Lucky runaway | One early random event swings game | Bound RNG; input randomness; deterministic early turns |
| Analysis paralysis | Turns take 5+ min | Cut branching; simultaneous selection; hide some info |
| First-player advantage | Seat 1 wins disproportionately | Bid for start; asymmetric starts; Stat Turn Order |
| Dead last / no agency | Trailing player cannot affect outcome | Catch-the-leader; temporary VP grabs; interaction that scales |
...
| NanDeck | Scripted card generation, free, batch output |
...
## Cross-References
- Workflow: `workflow.md` Stages 1 & 4
- Playtest triage: `playtesting.md` Framework 4
- Project log: `templates/balance-notes.md`
- Symptom routing: `cheatsheet.md` → Symptom → File table
